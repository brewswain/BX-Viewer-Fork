...
- 🎭 Theater Mode: Unobstructed viewing experience for maximum immersion! On by default (`T` or `Esc` leaves it); turn the default off in Settings. The playlist slides in from the right as a drawer — `P`, or the list button in the control bar. `Shift+T` swaps to the classic layout instead, where the page stays put and the playlist sits under the player beside the title, so there is no drawer to open. The picture is fitted to fill the stage rather than sit in fat side bars: it is stretched, and then zoomed, only as far as looks right, and whatever is left over stays as bars.
